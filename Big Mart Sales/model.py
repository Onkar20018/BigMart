...
item_weight_mean = dt.pivot_table(
...
outlet_size_mode = dt.pivot_table(
...
dt.loc[:, 'Item_Visibility'].replace(
    [0], [dt['Item_Visibility'].mean()], inplace=True)

dt['Item_Fat_Content'] = dt['Item_Fat_Content'].replace(
    {'LF': 'Low Fat', 'reg': 'Regular', 'low fat': 'Low Fat'})

dt['New_Item_Type'] = dt['Item_Identifier'].apply(lambda x: x[:2])
dt['New_Item_Type'] = dt['New_Item_Type'].map(
    {'FD': 'Food', 'NC': 'Non-Consumable', 'DR': 'Drinks'})
dt.loc[dt['New_Item_Type'] == 'Non-Consumable',
...
X = dt.drop(columns=['Item_Identifier', 'Outlet_Identifier',
            'Item_Outlet_Sales', 'Outlet_Establishment_Year'], axis=1)
y = dt['Item_Outlet_Sales']

X_copy = X

X_train, X_test, y_train, y_test = train_test_split(
    X_copy.values, y.values, test_size=0.3, random_state=0)
